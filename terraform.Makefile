...
$(1)-terraform-import:
	$(ROOT)/docker_run.py --aws -- \
		--volume $(ROOT):$(ROOT) \
		--workdir $(ROOT)/$(2) \
		$(TERRAFORM_DOCKER_IMAGE) import $(filter-out $(1)-terraform-import,$(MAKECMDGOALS))

$(1)-terraform-state-rm:
	$(ROOT)/docker_run.py --aws -- \
		--volume $(ROOT):$(ROOT) \
		--workdir $(ROOT)/$(2) \
		$(TERRAFORM_DOCKER_IMAGE) state rm $(filter-out $(1)-terraform-state-rm,$(MAKECMDGOALS))
